...
PART_CONFIG_TYPE = Dict[
    Literal['format', 'model', 'parts'],
    Dict[str, str]
]
...
class CarConfigs:
    ...
    configs: Dict[str, PART_CONFIG_TYPE] = field(init=False, default_factory=lambda: {}, repr=False)
    ...
    def find(
        ...
        cls._find_part_configs(
            beamng_path=beamng_path
        )
    ...
    def _find_part_configs(
        ...
        car_zips = [v for v in veh_zips if os.path.split(v)[1].replace(".zip", "") in CARS]
        if len(car_zips) == 0:
        ...
        for car_zip_path in tqdm(car_zips, desc="Searching for cars and configs."):
            ...
            with tempfile.TemporaryDirectory() as td, zipfile.ZipFile(car_zip_path, "r") as cz:
                cz.extractall(td)
        ...
        print(f"Successfully converted {success}/{found} of the found .pc files to .json.")
